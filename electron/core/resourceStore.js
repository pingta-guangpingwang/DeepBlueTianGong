...
function getRepoList() {
    return repoConfig_1.repoConfig.getVisibleRepos().map(r => r.name);
}
// ---- ResourceStore ----
class ResourceStore {
    constructor() {
        ...
        this.aiManifests = new Map();
        this.aiIndexes = new Map();
        this._lastLoadTime = 0;
        this._localChangesCache = new Map();
        this._changesCacheTTL = 3000;
        ...
        for (const repo of repos) {
            ...
            for (const item of items) {
                if (item.score < minScore)
                    continue;
                if (!this.matchesFilters(item, params))
                    continue;
                if (!query) {
    ...
    calcRelevance(item, query) {
        ...
        for (const word of queryWords) {
            ...
            if (item.name.toLowerCase().includes(word))
                score += 1;
            if (item.summary.toLowerCase().includes(word))
                score += 0.5;
            for (const t of item.tech_stack || []) {
                ...
                try {
                    ...
                    if (fs.existsSync(filePath)) {
                        ...
                        if (fmMatch) {
                            ...
                            if (fm) {
                                if (!item.summary_en && fm.summary_en)
                                    item.summary_en = String(fm.summary_en).replace(/\s+/g, ' ').trim();
                                if (!item.facets && fm.facets)
                                    item.facets = fm.facets;
                                item.rawYaml = fm;
                            }
                        }
                        else {
    ...
    async syncRepo(repo) {
        ...
        if (!fetchR.ok)
            return { success: false, message: `Fetch 失败: ${fetchR.output}`, pulled: 0 };
    ...
    /** 检查是否有远程更新 */
    async checkForUpdates(repo) {
        if (!await this.isGitRepo(repo)) {
            return { success: false, hasUpdates: false, behind: 0, message: '不是 Git 仓库' };
        }
        const fetchR = await this.gitSilent(repo, 'fetch origin --quiet', 30000);
        if (!fetchR.ok)
            return { success: false, hasUpdates: false, behind: 0, message: `Fetch 失败: ${fetchR.output}` };
        const branch = await this.git(repo, 'rev-parse --abbrev-ref HEAD').catch(() => 'main');
    ...
    // ============ 贡献流程 ============
    /** 获取本地变更文件列表 */
    async getLocalChanges(repo) {
        const repoPath = getRepoPath(repo);
        if (!fs.existsSync(path.join(repoPath, '.git')))
            return [];
        const cached = this._localChangesCache.get(repo);
        if (cached && Date.now() - cached.time < this._changesCacheTTL)
            return cached.data;
        try {
            const { execSync } = await import('child_process');
    ...
    async validateNewFileYaml(repo, filePath) {
        const errors = [];
        const repoPath = getRepoPath(repo);
        ...
        try {
            ...
            const REQUIRED_FIELDS = repoConfig_1.repoConfig.getRequiredFields(repo);
            for (const field of REQUIRED_FIELDS) {
                if (!fm[field] || (typeof fm[field] === 'string' && fm[field].trim() === '')) {
                    errors.push(`文件 ${filePath} 缺少必填字段: ${field}`);
                }
            }
            const VALID_TYPES = repoConfig_1.repoConfig.getValidTypes(repo);
    ...
    async validateCommitScope(repo) {
        const errors = [];
        ...
        if (blockedFiles.length > 0) {
            ...
            errors.push(`发现 ${blockedFiles.length} 个不允许提交的文件（仅允许新增文件，不支持修改或删除已有文件）:\n${fileList}`);
        }
    ...
    /** 创建贡献分支 */
    async createContributionBranch(repo, branchName) {
        if (!await this.isGitRepo(repo)) {
            return { success: false, message: '不是 Git 仓库' };
        }
        const changes = await this.getLocalChanges(repo);
        ...
        if (!syncR.success)
            return { success: false, message: `同步失败: ${syncR.message}` };
        const r = await this.gitSilent(repo, `checkout -b ${branchName}`);
        if (!r.ok)
            return { success: false, message: `创建分支失败: ${r.output}` };
    ...
    async commitAll(repo, message) {
        ...
        if (!addR.ok)
            return { success: false, message: `Git add 失败: ${addR.output}` };
        const commitR = await this.gitSilent(repo, `commit -m "${message}"`);
        if (!commitR.ok)
            return { success: false, message: `提交失败: ${commitR.output}` };
        return { success: true, message: '提交成功' };
    }
    /** 提交变更 */
    async commitChanges(repo, message, files) {
        if (!await this.isGitRepo(repo)) {
            return { success: false, message: '不是 Git 仓库' };
        }
        let filesToAdd;
        ...
        for (const file of filesToAdd) {
            ...
            if (!addR.ok)
                return { success: false, message: `Git add 失败 (${file}): ${addR.output}` };
        }
    ...
    /** 推送分支 */
    async pushBranch(repo, branchName) {
        if (!await this.isGitRepo(repo)) {
            return { success: false, message: '不是 Git 仓库' };
        }
    ...
    async safePushBranch(repo, branchName) {
        if (!await this.isGitRepo(repo)) {
    ...
    /** 创建 Pull Request */
    async createPullRequest(repo, branchName, title, body) {
        if (!await this.isGitRepo(repo)) {
            return { success: false, message: '不是 Git 仓库' };
        }
        ...
        try {
            const { execSync } = await import('child_process');
            const output = execSync(`gh pr create --title "${escapedTitle}" --body "${escapedBody}" --base main --head ${branchName}`, { cwd: repoPath, encoding: 'utf-8', timeout: 30000 });
    ...
    // ============ 初始化 ============
    /** 克隆仓库到本地 */
    async cloneRepo(repo, remoteUrl) {
        const repoPath = getRepoPath(repo);
        if (fs.existsSync(path.join(repoPath, '.git'))) {
            return { success: true, message: '仓库已存在' };
        }
        const cfg = repoConfig_1.repoConfig.getRepo(repo);
        ...
        for (const repo of repos) {
            ...
            if (item) {
                ...
                try {
                    ...
                    await fs.writeFile(path.join(getRepoPath(repo), 'manifest.json'), JSON.stringify(manifest, null, 2), 'utf-8');
                }
                catch { /* 写回失败不影响内存状态 */ }
                return;
            }
    ...
    /** 获取评分排行榜 */
    getLeaderboard(category, limit = 20) {
    ...
    matchesFilters(item, params) {
        ...
        if (params.techStack && params.techStack.length > 0) {
            const hasMatch = params.techStack.some(t => item.tech_stack.some(it => it.toLowerCase().includes(t.toLowerCase())));
            if (!hasMatch)
                return false;
        }
        return true;
...
exports.resourceStore = new ResourceStore();
